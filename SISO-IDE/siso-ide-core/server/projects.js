      ...
      if (line.trim()) {
        ...
        try {
          ...
          if (entry.sessionId) {
            ...
            // Track first activity for duration calculation
            const entryTime = entry.timestamp ? new Date(entry.timestamp) : new Date();
            if (!session.firstActivity || entryTime < session.firstActivity) {
              session.firstActivity = entryTime;
            }
            ...
            if (entry.message) {
              if (entry.message.role === 'user') {
                ...
                if (entry.message.content && Array.isArray(entry.message.content)) {
                  ...
                  session.toolUseCount += toolUses.length;
                }
              }
            }
            
            // Estimate token count from content length (rough approximation: 1 token ≈ 4 characters)
            if (entry.message?.content) {
              let contentLength = 0;
              if (typeof entry.message.content === 'string') {
                contentLength = entry.message.content.length;
              } else if (Array.isArray(entry.message.content)) {
                contentLength = entry.message.content.reduce((sum, block) => {
                  if (block.text) return sum + block.text.length;
                  if (block.input) return sum + JSON.stringify(block.input).length;
                  return sum;
                }, 0);
              }
              session.totalTokens += Math.ceil(contentLength / 4);
            }
            ...
            if (entry.timestamp) {
              ...
              if (session.firstActivity) {
                session.duration = session.lastActivity - session.firstActivity;
              }
            }
          }
        } catch (parseError) {
