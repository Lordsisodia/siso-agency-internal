...
import os from 'os';
import { trackClaudeUsage, trackSessionStart, trackSessionEnd } from './usageTracker.js';
...
/**
 * Enhanced event classification for real-time chat updates
 */
function classifyAndSendEvent(response, ws, sessionId) {
  // Always send original response for backward compatibility
  ws.send(JSON.stringify({
    type: 'claude-response',
    data: response
  }));

  // Enhanced real-time event classification
  console.log('🔍 Event classification - response.type:', response.type);
  if (response.type) {
    switch (response.type) {
      ...
      case 'content_block_start':
        if (response.content_block?.type === 'tool_use') {
          const toolName = response.content_block.name;
          ws.send(JSON.stringify({
            type: 'tool-started',
            sessionId: sessionId,
            tool: toolName,
            toolId: response.content_block.id,
            message: getToolStartMessage(toolName),
            icon: getToolIcon(toolName)
          }));
        } else if (response.content_block?.type === 'text') {
          ws.send(JSON.stringify({
            type: 'thinking-start',
            sessionId: sessionId,
            message: 'Thinking...'
      ...
      case 'content_block_stop':
        if (response.content_block?.type === 'tool_use') {
          ws.send(JSON.stringify({
            ...
            message: `Executing ${response.content_block.name}...`
          }));
        }
        break;
      ...
      case 'message_delta':
        if (response.delta?.stop_reason) {
          ws.send(JSON.stringify({
            type: 'claude-step-complete',
            sessionId: sessionId,
            reason: response.delta.stop_reason
          }));
        }
        break;
          ...
          response.content.forEach(content => {
            if (content.type === 'tool_result') {
              ws.send(JSON.stringify({
                ...
                message: content.is_error ? 'Tool execution failed' : 'Tool completed successfully',
  ...
  // Handle status and progress events
  if (response.status) {
    ws.send(JSON.stringify({
      type: 'claude-status',
      sessionId: sessionId,
...
function getToolStartMessage(toolName) {
  const messages = {
    ...
    'Task': 'Spawning specialized agent...',
    'TodoWrite': 'Updating task list...',
    'WebSearch': 'Searching the web...',
    'WebFetch': 'Fetching web content...'
  };
  return messages[toolName] || `Using ${toolName} tool...`;
}
...
async function spawnClaude(command, options = {}, ws) {
  return new Promise(async (resolve, reject) => {
    ...
    let model = 'claude-3.5-sonnet'; // Default model
    if (!resume) {
      args.push('--model', 'sonnet');
      model = 'claude-3.5-sonnet'; // Map CLI model name to full model name
    }
        ...
        try {
          ...
          if (response.session_id && !capturedSessionId) {
            ...
            if (!sessionId && !sessionCreatedSent) {
              ...
              // Track session start
              trackSessionStart(capturedSessionId, {
                model: model,
                projectPath: projectPath || cwd,
                permissionMode: permissionMode,
                hasMcp: args.includes('--mcp-config'),
          ...
          // Track Claude usage if response contains usage data
          if (response.usage || (response.response && response.response.usage)) {
            trackClaudeUsage(response, {
              sessionId: capturedSessionId || sessionId,
              model: model,
              projectPath: projectPath || cwd,
              promptLength: command ? command.length : 0,
              hasAttachments: images && images.length > 0
            });
          }
          
          // Enhanced event classification for real-time updates
          classifyAndSendEvent(response, ws, capturedSessionId || sessionId);
        } catch (parseError) {
    ...
    claudeProcess.on('close', async (code) => {
      ...
      // Track session end
      if (finalSessionId && finalSessionId !== 'unknown') {
        trackSessionEnd(finalSessionId, {
          exitCode: code,
