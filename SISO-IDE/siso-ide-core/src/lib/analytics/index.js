  ...
  sanitizeProperties(properties) {
    ...
    for (const [key, value] of Object.entries(properties)) {
      ...
      if (key.includes('path') || key.includes('file')) {
        sanitized[key] = typeof value === 'string' ? sanitizeFilePath(value) : value;
      } else if (key.includes('project')) {
        sanitized[key] = typeof value === 'string' ? sanitizeFilePath(value) : value;
      } else if (key.includes('error') || key.includes('message')) {
        sanitized[key] = typeof value === 'string' ? sanitizeErrorMessage(value) : value;
    ...
    events.forEach(event => {
      if (typeof posthog !== 'undefined' && posthog.capture) {
        posthog.capture(event.event, {
          ...
          $current_url: `siso-ide://${event.properties?.screen_name || 'unknown'}`,
        });
      }
    });
  }
...
export const performanceTracker = PerformanceTracker.instance;