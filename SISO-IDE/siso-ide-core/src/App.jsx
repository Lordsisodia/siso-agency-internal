...
import UITestRoute from './components/UITestRoute';
...
function AppContent() {
  ...
  // activeTab removed - unified sidebar handles all navigation
  ...
  const [currentPage, setCurrentPage] = useState('main'); // 'main', 'settings', 'usage'
  ...
  // Multi-tab feature flag (proof of concept)
  const [showActiveSessionsPanel, setShowActiveSessionsPanel] = useState(() => {
    ...
    return saved !== null ? JSON.parse(saved) : false; // Start disabled
  });
    ...
    const initializeAnalytics = async () => {
      try {
        ...
        analytics.track('app_started', {
          ...
          app_version: '1.7.0' // From package.json
        });
  ...
  // Toggle active sessions panel and persist preference
  const toggleActiveSessionsPanel = () => {
    const newValue = !showActiveSessionsPanel;
    setShowActiveSessionsPanel(newValue);
    localStorage.setItem('showActiveSessionsPanel', JSON.stringify(newValue));
  };
        ...
        <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 w-full max-w-md mx-4 p-6 space-y-4">
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs text-gray-600 dark:text-gray-400">
              Run this command in your SISO directory to update to the latest version.
            </p>
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {currentPage === 'main' && (
          <SimplifiedMainContent
            ...
            showActiveSessionsPanel={showActiveSessionsPanel}
            onToggleActiveSessionsPanel={toggleActiveSessionsPanel}
        ...
        {currentPage === 'usage' && (
          <UsagePage onBack={handleBackToMain} />
        )}
      </div>

      {/* Mobile Bottom Navigation - Simplified for chat focus */}
      {isMobile && (
        ...
        <ProtectedRoute>
          <Router>
            <Routes>
              ...
              <Route path="/ui-test" element={<UITestRoute />} />
            </Routes>
          </Router>
        </ProtectedRoute>
      </AuthProvider>
