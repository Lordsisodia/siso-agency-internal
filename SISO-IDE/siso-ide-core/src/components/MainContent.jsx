...
import SISOBrandTest from './SISOBrandTest';
...
function MainContent({ 
  ...
  showActiveSessionsPanel, // Show the active sessions panel (proof of concept)
  onToggleActiveSessionsPanel, // Toggle the active sessions panel
  projects = [], // All projects data for the sessions panel
  ...
  const [terminalManagerRef, setTerminalManagerRef] = useState(null);
  ...
  // Terminal favorites handlers
  const handleSwitchToTerminal = (terminalId) => {
    if (terminalManagerRef) {
      terminalManagerRef.switchToTerminal(terminalId);
      setActiveTab('shell'); // Switch to shell tab
    }
  };

  const handleRestartTerminal = (terminalId) => {
    if (terminalManagerRef) {
      terminalManagerRef.restartTerminal(terminalId);
    }
  };

  const handleTerminalStateChange = (terminalId, terminal) => {
      ...
      <div className="h-full flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center text-gray-500 dark:text-gray-400">
            ...
            <h2 className="text-xl font-semibold mb-2">Loading SISO</h2>
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 p-3 sm:p-4 flex-shrink-0">
        <div className="flex items-center justify-between">
          ...
          {/* Right side: Toggle button for active sessions panel */}
          <div className="flex items-center">
            <button
              onClick={onToggleActiveSessionsPanel}
              className={`p-2 rounded-md transition-colors ${
                showActiveSessionsPanel 
                  ? 'bg-[hsl(var(--siso-orange))] text-[hsl(var(--siso-black))]' 
                  : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700'
              }`}
              title={showActiveSessionsPanel ? 'Hide active sessions' : 'Show active sessions'}
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
