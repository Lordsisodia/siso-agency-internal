...
function ChatInterface({ selectedProject, selectedSession, ws, sendMessage, messages, onFileOpen, onInputFocusChange, onSessionActive, onSessionInactive, onReplaceTemporarySession, onNavigateToSession, onShowSettings, autoExpandTools, showRawParameters, autoScrollToBottom, sendByCtrlEnter }) {
  ...
  // Real-time activity and thinking state
  ...
  const [showPromptQueue, setShowPromptQueue] = useState(() => {
    return JSON.parse(localStorage.getItem('showPromptQueue') || 'false');
  });
      ...
      switch (latestMessage.type) {
        case 'session-created':
          ...
          if (latestMessage.sessionId && !currentSessionId) {
            ...
            // Analytics: Track new session creation
            try {
              analytics.track('session_created', {
                provider: provider,
                session_id: latestMessage.sessionId,
                project_path: selectedProject?.fullPath || selectedProject?.path,
        ...
        case 'claude-response':
          ...
          // Extract and update token usage from Claude response 
          if (messageData && messageData.message && messageData.message.usage) {
            ...
            setClaudeStatus(prev => ({
              ...prev,
              tokens: totalTokens,
              text: prev?.text || 'Processing'
            }));
          }
          
          // Handle Claude CLI final result with cumulative token usage
          if (messageData && messageData.type === 'result' && messageData.usage) {
          ...
          // Handle Claude CLI response format: the content is in data.message.content
          if (messageData && messageData.message) {
            const claudeMessage = messageData.message;
            
            // Process Claude response content
            if (Array.isArray(claudeMessage.content)) {
              for (const part of claudeMessage.content) {
                if (part.type === 'tool_use') {
                  ...
                  // Add regular text message and clear any streaming buffer
            ...
            } else if (typeof claudeMessage.content === 'string' && claudeMessage.content.trim()) {
              // Normalize usage limit message to local time
              let content = formatUsageLimitText(claudeMessage.content);
            ...
            break; // Exit for Claude CLI format
          }
          
          // Handle Cursor streaming format (content_block_delta / content_block_stop) - fallback for old format
        ...
        case 'claude-output':
          // Ignore banner and non-JSON output from Claude CLI
          // These are startup messages that should not be displayed as chat content
          break;
        ...
        case 'claude-complete':
          ...
          // Analytics: Track conversation completion
          try {
            const conversationLength = chatMessages.filter(msg => msg.type === 'user' || msg.type === 'assistant').length;
            analytics.track('conversation_completed', {
              provider: provider,
              session_id: activeSessionId,
              project_path: selectedProject?.fullPath || selectedProject?.path,
              conversation_length: conversationLength,
              exit_code: latestMessage.exitCode,
              successful: latestMessage.exitCode === 0,
              duration_estimate: Math.floor((new Date() - new Date(chatMessages[0]?.timestamp || new Date())) / 1000) // seconds
            });
          } catch (error) {
            console.error('Analytics tracking error:', error);
          }
        ...
        case 'session-aborted':
          ...
          // Analytics: Track conversation abort
          try {
            const conversationLength = chatMessages.filter(msg => msg.type === 'user' || msg.type === 'assistant').length;
            analytics.track('conversation_aborted', {
              provider: provider,
        ...
        case 'tool-started':
          console.log('🔧 Frontend received tool-started:', latestMessage);
          setActivities(prev => [...prev, {
            type: 'tool-started',
            tool: latestMessage.tool,
            toolId: latestMessage.toolId,
            message: latestMessage.message,
            icon: latestMessage.icon,
            timestamp: Date.now()
          }]);
          setActiveTools(prev => new Map(prev.set(latestMessage.toolId, {
            tool: latestMessage.tool,
            status: 'started',
            startTime: Date.now()
          })));
        ...
        case 'tool-waiting':
          ...
          setActiveTools(prev => {
            ...
            const existing = newMap.get(latestMessage.toolId);
            if (existing) {
              newMap.set(latestMessage.toolId, { ...existing, status: 'executing' });
            }
            return newMap;
          });
          break;
        ...
        case 'tool-completed':
          setActivities(prev => [...prev, {
            ...
            toolId: latestMessage.toolId,
            message: latestMessage.message,
            success: latestMessage.success,
            result: latestMessage.result,
            timestamp: Date.now()
          }]);
          setActiveTools(prev => {
            const newMap = new Map(prev);
            newMap.delete(latestMessage.toolId);
            return newMap;
          });
          break;
        ...
        case 'thinking-stream':
          console.log('🧠 Frontend received thinking-stream:', latestMessage.content);
          setThinkingContent(prev => prev + latestMessage.content);
          break;
        ...
        case 'claude-thinking-start':
          ...
          setThinkingContent('');
          setActivities(prev => [...prev, {
            type: 'claude-thinking-start',
            message: latestMessage.message,
            timestamp: Date.now()
          }]);
          break;

        case 'claude-message-complete':
          setIsThinking(false);
          // Clear activities after a delay to show completion
          setTimeout(() => {
            setActivities([]);
  ...
  // Handle sending prompts from the queue
  const handleSendPrompt = useCallback(async (promptText) => {
    if (!promptText.trim() || isLoading || !selectedProject) return;
    
    // Don't modify the input field when sending from queue
    const fakeEvent = { preventDefault: () => {} };
    handleSubmitInternal(fakeEvent, promptText, false); // false = don't clear input
  ...
  const handleSubmit = async (e) => {
    ...
    handleSubmitInternal(e, input, true); // true = clear input after sending
  };

  const handleSubmitInternal = async (e, promptText, shouldClearInput = true) => {
    if (!promptText.trim() || isLoading || !selectedProject) return;
    ...
    try {
      analytics.track('chat_message_sent', {
        ...
        message_length: promptText.length,
        project_path: selectedProject?.fullPath || selectedProject?.path,
        session_id: sessionToActivate,
        is_new_session: !effectiveSessionId,
        has_session: !!effectiveSessionId
      });
    } catch (error) {
      console.error('Analytics tracking error:', error);
    }
    ...
    // Clear input and reset state (only clear input if it was a direct form submission)
    if (shouldClearInput) {
      setInput('');
      setAttachedImages([]);
      ...
      <div className={`p-2 sm:p-4 md:p-4 flex-shrink-0 ${
        ...
        <div className="max-w-4xl mx-auto mb-3">
          <div className="flex items-center justify-center gap-3">
            ...
            <button
              onClick={() => {
                const newValue = !showPromptQueue;
                setShowPromptQueue(newValue);
                localStorage.setItem('showPromptQueue', JSON.stringify(newValue));
              }}
              className={`w-8 h-8 ${showPromptQueue ? 'bg-green-600 hover:bg-green-700' : 'bg-gray-600 hover:bg-gray-700'} text-white rounded-full shadow-lg flex items-center justify-center transition-all duration-200 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 dark:ring-offset-gray-800`}
              title={showPromptQueue ? "Hide Prompt Queue" : "Show Prompt Queue"}
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
