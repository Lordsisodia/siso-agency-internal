...
import { isBrowserOnline } from '@/ecosystem/internal/tasks/utils/network';
...
} from '@/ecosystem/internal/tasks/hooks/deepWork/deepWorkTaskCache';
...
export function useClientDeepWorkTasks({ clientId }: UseClientDeepWorkTasksProps) {
  ...
  const addSubtask = useCallback(async (taskId: string, subtaskTitle: string, priority = 'MEDIUM') => {
    ...
    try {
      ...
      const newSubtask: import('@/ecosystem/internal/tasks/hooks/useDeepWorkTasksSupabase').DeepWorkSubtask = {
        id: data.id,
        taskId: data.task_id,
        title: data.title,
