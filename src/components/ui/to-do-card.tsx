...
export function TodoCard({ className }: TodoCardProps) {
  ...
  const commitEdit = () => {
    ...
    setItems((prev) =>
      prev.map((item) =>
        item.id === editingId
          ? { ...item, text: draftText.trim().length ? draftText.trim() : item.text }
          : item,
      ),
    );
    setEditingId(null);
  ...
  const addItem = () => {
    ...
    const newItem = { id: nextId, text: 'New task', done: false };
  ...
  return (
    <div
      className={[
        'w-full rounded-2xl shadow-lg border overflow-hidden bg-white transition-all duration-500',
        allDone ? 'border-emerald-200 ring-2 ring-emerald-200 scale-[1.005]' : 'border-slate-100',
        className ?? '',
      ]
        .filter(Boolean)
        .join(' ')}
          ...
          <>
                    ...
                    <div className="flex-1 min-w-0">
                      {isEditing ? (
                        <input
                          ...
                          className="w-full rounded-md border border-gray-200 px-2 py-1 text-sm text-gray-900 outline-none ring-2 ring-transparent focus:ring-gray-900/20"
                        />
                      ) : (
                        <button
                          type="button"
                          onClick={() => startEditing(item.id, item.text)}
                          className={`text-left text-sm transition-all duration-200 w-full ${
                            item.done
                              ? 'font-semibold text-gray-900 translate-x-[2px]'
                              : 'text-gray-900'
                          }`}
                        >
                          {item.text}
            ...
            <div className="mt-4 flex items-center gap-2">
              <button
                type="button"
                onClick={addItem}
                className="inline-flex items-center gap-2 rounded-md border border-gray-200 bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm transition hover:border-gray-300 hover:bg-gray-50 active:scale-[0.99]"
              >
                <span className="flex h-5 w-5 items-center justify-center rounded-full bg-gray-900 text-white text-xs leading-none">
                  +
                </span>
                Add task
              </button>
              <p className="text-xs text-gray-500">Click a task to edit; Enter to save, Esc to cancel.</p>
            </div>
          </>
        )}
