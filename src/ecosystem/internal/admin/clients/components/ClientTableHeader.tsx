  ...
  return (
    <TableHeader className="sticky top-0 z-30">
      <TableRow className="hover:bg-transparent border-b border-white/10 bg-[#0F0E16]/95 backdrop-blur-sm">
        <TableHead className="sticky left-0 top-0 z-40 h-10 w-10 bg-[#0F0E16]/95 px-2 backdrop-blur-sm">
          <Checkbox 
        ...
        {visibleColumns.map((column, index) => {
          ...
          let leftPosition = 36; // Adjusted for checkbox column width
          if (isPinned) {
          ...
          return (
            <TableHead 
              key={column.key}
              className={cn(
                "sticky top-0 z-30 h-10 text-xs font-medium uppercase tracking-wider text-white/70 bg-[#0F0E16]/95 backdrop-blur-sm border-b border-white/10",
                isPinned ? 'z-40' : ''
              )}
              style={{ 
        ...
        <TableHead
          className="sticky top-0 right-0 z-30 h-10 w-24 bg-[#0F0E16]/95 text-xs font-medium uppercase tracking-wider text-white/60 backdrop-blur-sm"
        >
          Actions
        </TableHead>
      </TableRow>
