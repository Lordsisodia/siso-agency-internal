...
import type { WaterTrackerSnapshot } from './types';
...
export const WaterTracker: React.FC<WaterTrackerProps> = ({ selectedDate, userId }) => {
  ...
  const handleAmountChange = async (amount: number) => {
    ...
    const currentTotal = data?.dailyTotalMl ?? 0;
  ...
  const displaySnapshot = data ?? {
    ...
    entries: [],
    historyTotals: {},
  } satisfies WaterTrackerSnapshot;
              ...
              {WATER_ACTION_AMOUNTS.map(amount => {
                ...
                return (
                  <Button
                    ...
                    disabled={!userId || isSubmitting || isLoading}
