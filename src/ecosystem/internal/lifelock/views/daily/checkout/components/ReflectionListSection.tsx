...
interface ReflectionListSectionProps {
  title: string;
  items: string[];
...
export const ReflectionListSection: React.FC<ReflectionListSectionProps> = ({
  title,
  items,
  onChange,
  onAdd,
  ...
  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 + addDelay }} className="space-y-3">
    <div className="flex items-center justify-between">
      <h4 className="font-semibold text-purple-300 text-base">{title}</h4>
      {onAdd && (
        <Button onClick={onAdd} size="sm" variant="ghost" className="text-purple-400 hover:text-purple-300 hover:bg-purple-900/20 h-8">
