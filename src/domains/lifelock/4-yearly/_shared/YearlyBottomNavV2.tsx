/**
 * YearlyBottomNavV2 Component
 *
 * Shared bottom navigation with glassmorphism effect for LifeLock yearly views.
 * Based on WeeklyBottomNavV2 pattern with orange theme for yearly.
 *
 * 3-pill navigation + More button (9-dot) as 4th pill
 * - Review (Overview, Wins, Vision)
 * - Goals (Goals, Milestones, Growth)
 * - Balance (Scorecard, Trends, Insights)
 */
...
export interface YearlyBottomNavTab {
  title: string;
  icon: LucideIcon;
  color?: string;
  bgActive?: string;
...
export interface YearlyBottomNavV2Props {
  tabs: YearlyBottomNavTab[];
  activeIndex: number;
  activeColor?: string;
...
export const YearlyBottomNavV2: React.FC<YearlyBottomNavV2Props> = ({
  tabs,
  activeIndex,
  activeSubTab,
  onChange,
  ...
  // Get color based on active subsection - ORANGE THEME for Yearly
  ...
  // ORANGE THEME for Yearly view
  const tabColors = [
    'from-orange-500 to-amber-500',     // Review
    'from-amber-500 to-orange-500',     // Goals
    'from-rose-500 to-orange-500',      // Balance
  ];
