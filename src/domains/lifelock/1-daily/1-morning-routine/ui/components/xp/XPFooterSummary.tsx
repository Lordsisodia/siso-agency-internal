...
export const XPFooterSummary: React.FC<XPFooterSummaryProps> = ({ breakdown, totalXP }) => {
  ...
  return (
    <motion.div
      ...
      className="mt-6"
    >
      <Card className="bg-gradient-to-br from-orange-900/20 to-orange-900/20 border-orange-700/40">
          ...
          <div className="space-y-2 text-sm">
            ...
            {breakdown.wakeUp > 0 && (
              <div className="flex items-center justify-between">
                <span className="text-orange-100/90">Wake Up</span>
                <span className="text-orange-300 font-bold">{breakdown.wakeUp} XP 💎</span>
              </div>
            )}
            ...
            {breakdown.getBloodFlowing > 0 && (
              <div className="flex items-center justify-between">
                <span className="text-orange-100/90">
                  Get Blood Flowing
                  {breakdown.bonuses.pushupPB && <span className="text-xs text-orange-300 ml-2">🏆 +{breakdown.bonuses.pushupPB} PB</span>}
                  {breakdown.bonuses.pushupSpeed && <span className="text-xs text-green-300 ml-2">⚡ +{breakdown.bonuses.pushupSpeed} speed</span>}
                </span>
                <span className="text-orange-300 font-bold">{breakdown.getBloodFlowing} XP 💪</span>
              </div>
            )}
            ...
            {breakdown.planDay > 0 && (
              <div className="flex items-center justify-between">
                <span className="text-orange-100/90">Plan Day</span>
            ...
            {breakdown.meditation > 0 && (
              <div className="flex items-center justify-between">
                <span className="text-orange-100/90">Meditation</span>
                <span className="text-orange-300 font-bold">{breakdown.meditation} XP 🧘</span>
              </div>
            )}

            {breakdown.priorities > 0 && (
              <div className="flex items-center justify-between">
                <span className="text-orange-100/90">Top 3 Priorities</span>
                <span className="text-green-300 font-bold">{breakdown.priorities} XP 🎯</span>
              </div>
            )}

            {/* Total */}
            <div className="border-t border-orange-600/30 pt-2 mt-3">
              <div className="flex items-center justify-between">
                <span className="text-orange-200 font-semibold">Total XP Today</span>
                <span className="text-orange-300 font-bold text-lg">{totalXP} XP 🏆</span>
              </div>
            </div>

            {/* Bonus summary */}
            {hasBonuses && (
              <div className="mt-3 p-3 bg-orange-900/10 border border-orange-600/20 rounded-lg">
                <div className="flex items-center gap-2 mb-2">
                  <Target className="h-4 w-4 text-green-400" />
                  <span className="text-xs text-orange-200 font-medium">Bonus XP Earned</span>
                </div>
                <div className="space-y-1 text-xs text-orange-100/80">
