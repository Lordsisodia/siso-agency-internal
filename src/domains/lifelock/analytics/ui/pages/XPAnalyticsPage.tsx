/**
 * XP Analytics Page - Redesigned
 *
 * Beautiful glassmorphic dashboard with proper styling
 */
...
import { CategoryBreakdownCard } from '../components/categories/CategoryBreakdownCard';
import { PersonalBestsCard } from '../components/records/PersonalBestsCard';
import { StreakCard } from '../components/records/StreakCard';
import { TrendChartCard } from '../components/trends/TrendChartCard';
import { PeakProductivityCard } from '../components/productivity/PeakProductivityCard';
import { RecentAchievementsCard } from '../components/achievements/RecentAchievementsCard';
import { XPBottomNav } from './XPBottomNav';
...
export function XPAnalyticsPage() {
  ...
  const handleTabChange = (index: number | null) => {
    if (index === null) return; // More menu - handle separately
    setActiveTab(XP_HUB_TABS[index].id as XPHubTab);
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950 p-4 pb-32">
        ...
        {/* Bottom Nav Skeleton */}
        <div className="fixed bottom-6 left-4 right-4 h-16 bg-white/10 rounded-full animate-pulse" />
      </div>
    );
  }
        ...
        <div className="mt-4 p-4 bg-white/5 border border-white/10 rounded-2xl">
          ...
          <div className="flex justify-between mt-2">
            <span className="text-xs text-gray-500">
              {analytics.level.xpToNext} XP to next level
            </span>
            <span className="text-xs text-gray-500">
              ~{analytics.level.estimatedDaysToNextLevel} days
            </span>
          </div>
        </div>
      </motion.div>
          ...
          >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <CategoryBreakdownCard
                categories={analytics.today.categories}
                totalXP={analytics.today.total}
              />
