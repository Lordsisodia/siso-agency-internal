...
export const TimelineTaskView: React.FC<TimelineTaskViewProps> = ({ memberId }) => {
  ...
  useEffect(() => {
    // TODO: Replace mock data with real API once available
    const fetchData = async () => {
      ...
      const mockMarkers = [
        { id: "milestone-1", date: new Date(2025, 3, 20), label: "Design Review" },
        { id: "milestone-2", date: new Date(2025, 4, 5), label: "Prototype Demo" },
      ];
  ...
  const handleTaskMove = (id: string, startAt: Date, endAt: Date | null) => {
    setTasks((prev) =>
      prev.map((task) => (task.id === id ? { ...task, startAt, endAt: endAt ?? task.endAt } : task))
    );
  };

  if (loading) {
    return <div className="flex h-64 items-center justify-center">Loading timeline...</div>;
  }
      ...
      <Gantt range="daily" today={new Date()}>
        ...
        <GanttTimeline>
          ...
          <GanttFeatureList>
            <GanttFeatureListGroup>
              {tasks.map((task) => (
                <GanttFeatureItem key={task.id} {...task} onMove={handleTaskMove} />
              ))}
            </GanttFeatureListGroup>
            {markers.map((marker) => (
