...
import { api, type Agent } from "@/shared/lib/claudia-api";
...
import { AutoContinuationControls } from "./AutoContinuationControls";
import { AutoContinuationConfig } from "@/shared/lib/auto-continuation";
...
export const AgentExecution: React.FC<AgentExecutionProps> = ({
  ...
  const [autoContinuationConfig, setAutoContinuationConfig] = useState<AutoContinuationConfig | null>(null);
  const [currentIteration, setCurrentIteration] = useState(0);
  const [totalCost, setTotalCost] = useState(0);
  const [runIds, setRunIds] = useState<number[]>([]);
  
  // Use runIds to track cost across iterations
  React.useEffect(() => {
    if (runIds.length > 0) {
      // Could calculate total cost from all runs here
      setTotalCost(runIds.length * 0.01); // Placeholder calculation
    }
  }, [runIds]);
  ...
  const handleExecuteWithContinuation = async (config: AutoContinuationConfig) => {
    if (!projectPath || !task.trim()) return;
    ...
    try {
      ...
      setTotalCost(0);
      setAutoContinuationConfig(config);

      // Execute agent with auto-continuation
      const executedRunIds = await api.executeAgentWithContinuation(
        agent.id!,
        projectPath,
        task,
        config.enabled,
      ...
      setRunIds(executedRunIds);
      
      // Set up event listeners for all run IDs
      const unlistenPromises = executedRunIds.map(async (runId, index) => {
        const outputUnlisten = await listen<string>(`agent-output:${runId}`, (event) => {
          try {
            setRawJsonlOutput(prev => [...prev, event.payload]);
            const message = JSON.parse(event.payload) as ClaudeStreamMessage;
        ...
        const completeUnlisten = await listen<boolean>(`agent-complete:${runId}`, () => {
          if (index === executedRunIds.length - 1) {
            // Only set not running when the last iteration completes
            setIsRunning(false);
          }
        });

        return [outputUnlisten, errorUnlisten, completeUnlisten];
      });

      // Store all unlisten functions
      const allUnlistenFunctions = (await Promise.all(unlistenPromises)).flat();
      unlistenRefs.current.push(...allUnlistenFunctions);

    } catch (err) {
      console.error("Failed to execute agent with continuation:", err);
          ...
          <div className="w-full max-w-5xl mx-auto p-4 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  onKeyPress={(e) => {
                    if (e.key === "Enter" && !isRunning && projectPath && task.trim()) {
                      if (autoContinuationConfig?.enabled) {
                        handleExecuteWithContinuation(autoContinuationConfig);
                      } else {
                        handleExecute();
                      }
                    }
                ...
                />
                <Button
                  onClick={isRunning ? handleStop : (autoContinuationConfig?.enabled ? () => handleExecuteWithContinuation(autoContinuationConfig) : handleExecute)}
            ...
            {/* Auto-Continuation Controls */}
            <AutoContinuationControls
              onConfigChange={setAutoContinuationConfig}
              onExecuteWithContinuation={handleExecuteWithContinuation}
