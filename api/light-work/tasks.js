/**
 * Light Work Tasks API - Stub Handler
...
export default async function handler(req, res) {
  ...
  // Parse URL path to handle nested routes like /api/light-work/tasks/123/toggle
  const pathParts = url?.split('?')[0]?.split('/').filter(Boolean) || [];
  const taskId = pathParts[2]; // /api/light-work/tasks/{taskId}/...
