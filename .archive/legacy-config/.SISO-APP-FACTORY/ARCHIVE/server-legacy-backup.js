...
app.get('/api/tasks', async (req, res) => {
  try {
    ...
    console.log(`🎯 Loaded ${tasks.length} tasks for ${userId}`);
    
    // Debug: Log each task details
    tasks.forEach((task, index) => {
      console.log(`📋 Task ${index + 1}: "${task.title}" (${task.workType}) - ${task.subtasks.length} subtasks`);
      if (task.subtasks.length > 5) {
        console.log(`  📍 Has many subtasks - first one: "${task.subtasks[0]?.text}"`);
      }
    });
...
app.post('/api/subtasks/update-date', async (req, res) => {
  try {
    const { subtaskId, dueDate } = req.body;
    ...
    console.log(`📅 Updated subtask due date: ${updatedSubtask.title} -> ${dueDate || 'No due date'}`);
    res.json({ success: true, data: updatedSubtask });
  } catch (error) {
...
app.post('/api/subtasks/update-priority', async (req, res) => {
  try {
    const { subtaskId, priority } = req.body;

    if (!subtaskId || !priority) {
      return res.status(400).json({ error: 'subtaskId and priority are required' });
    }
    ...
    console.log(`🎯 Updated subtask priority: ${updatedSubtask.title} -> ${priority}`);
    res.json({ success: true, data: updatedSubtask });
  } catch (error) {
