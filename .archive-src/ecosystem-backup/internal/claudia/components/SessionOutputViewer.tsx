...
import { useOutputCache } from '@/shared/lib/outputCache';
import type { AgentRun } from '@/shared/lib/claudia-api';
...
export function SessionOutputViewer({ session, onClose, className }: SessionOutputViewerProps) {
  ...
  const loadOutput = async (skipCache = false) => {
    ...
    try {
      ...
      toast({ title: 'Error', description: 'Failed to load session output', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const setupLiveEventListeners = async () => {
    ...
    try {
      ...
      const errorUnlisten = await listen<string>(`agent-error:${session.id}`, (event) => {
        ...
        toast({ title: 'Error', description: event.payload, variant: 'destructive' });
      });

      const completeUnlisten = await listen<boolean>(`agent-complete:${session.id}`, () => {
        toast({ title: 'Success', description: 'Agent execution completed' });
      ...
      const cancelUnlisten = await listen<boolean>(`agent-cancelled:${session.id}`, () => {
        toast({ title: 'Error', description: 'Agent execution was cancelled', variant: 'destructive' });
      });
  ...
  const handleCopyAsJsonl = async () => {
    ...
    toast({ title: 'Success', description: 'Output copied as JSONL' });
  };
  ...
  const handleCopyAsMarkdown = async () => {
    ...
    setCopyPopoverOpen(false);
    toast({ title: 'Success', description: 'Output copied as Markdown' });
  };
  ...
  const refreshOutput = async () => {
    ...
    try {
      ...
      toast({ title: 'Success', description: 'Output refreshed' });
    } catch (error) {
      console.error('Failed to refresh output:', error);
      toast({ title: 'Error', description: 'Failed to refresh output', variant: 'destructive' });
    } finally {
      setRefreshing(false);
    }
  };
