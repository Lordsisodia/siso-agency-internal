...
import { api, type AgentRunWithMetrics } from '@/shared/lib/claudia-api';
...
import { formatISOTimestamp } from '@/shared/lib/date-utils';
...
export function AgentRunOutputViewer({ 
  ...
  const loadOutput = async (skipCache = false) => {
    ...
    try {
      ...
      toast({ title: 'Error', description: 'Failed to load agent output', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
