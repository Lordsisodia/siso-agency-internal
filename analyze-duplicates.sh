...
# Script to analyze duplicate files and their usage

echo "=== DUPLICATE COMPONENT ANALYSIS ===" > /tmp/duplicate-analysis.txt
echo "" >> /tmp/duplicate-analysis.txt

# Define arrays of duplicates
...
declare -a category_b
declare -a category_c
...
analyze_file() {
    ...
    echo "Analyzing: $name" >> /tmp/duplicate-analysis.txt

    # Check if component version exists
    if [ ! -f "src/$comp_path" ]; then
        echo "  SKIP: Component version not found" >> /tmp/duplicate-analysis.txt
        return
    fi

    # Check if domains version exists
    if [ ! -f "src/$dom_path" ]; then
        echo "  SKIP: Domains version not found" >> /tmp/duplicate-analysis.txt
        return
    fi

    # Count imports for component version
    local comp_imports=$(grep -r "from.*$comp_path" src/ 2>/dev/null | wc -l | tr -d ' ')
    local comp_imports_exact=$(grep -r "from '@/components/" src/ 2>/dev/null | grep -v "node_modules" | grep "$(basename $comp_path)" | wc -l | tr -d ' ')

    # Count imports for domains version
    local dom_imports=$(grep -r "from.*$dom_path" src/ 2>/dev/null | wc -l | tr -d ' ')
    local dom_imports_exact=$(grep -r "from '@/domains/" src/ 2>/dev/null | grep -v "node_modules" | grep "$(basename $dom_path)" | wc -l | tr -d ' ')

    echo "  Component ($comp_path): $comp_imports imports" >> /tmp/duplicate-analysis.txt
    echo "  Domains ($dom_path): $dom_imports imports" >> /tmp/duplicate-analysis.txt

    # Check if component file is a re-export
    if [ -f "src/$comp_path" ]; then
        local content=$(cat "src/$comp_path" 2>/dev/null)
        if echo "$content" | grep -q "export.*from.*@/domains/"; then
            echo "  -> CATEGORY B: Re-export pattern" >> /tmp/duplicate-analysis.txt
            category_b+=("$name:$comp_path:$dom_path")
            return
        fi
    fi
    ...
    if [ "$comp_imports" -eq 0 ] && [ "$dom_imports" -eq 0 ]; then
        echo "  -> CATEGORY A: Both unused (safe to delete component)" >> /tmp/duplicate-analysis.txt
        category_a+=("$name:$comp_path:$dom_path")
    elif [ "$comp_imports" -eq 0 ] && [ "$dom_imports" -gt 0 ]; then
        echo "  -> CATEGORY A: Component unused, domains used (safe to delete component)" >> /tmp/duplicate-analysis.txt
        category_a+=("$name:$comp_path:$dom_path")
    elif [ "$comp_imports" -gt 0 ] && [ "$dom_imports" -eq 0 ]; then
        echo "  -> CATEGORY C: Component used, domains unused (NEEDS REVIEW)" >> /tmp/duplicate-analysis.txt
        category_c+=("$name:$comp_path:$dom_path (component:$comp_imports, domains:0)")
    else
        echo "  -> CATEGORY C: Both used (NEEDS REVIEW)" >> /tmp/duplicate-analysis.txt
        category_c+=("$name:$comp_path:$dom_path (component:$comp_imports, domains:$dom_imports)")
    fi
    echo "" >> /tmp/duplicate-analysis.txt
}
...
for duplicate in "${duplicates[@]}"; do
    ...
    analyze_file "$name" "$comp_path" "$dom_path"
done

# Print summary
echo "=== SUMMARY ===" >> /tmp/duplicate-analysis.txt
echo "" >> /tmp/duplicate-analysis.txt
echo "CATEGORY A: Safe to Delete (${#category_a[@]} files)" >> /tmp/duplicate-analysis.txt
for item in "${category_a[@]}"; do
    echo "  - $item" >> /tmp/duplicate-analysis.txt
done

echo "" >> /tmp/duplicate-analysis.txt
echo "CATEGORY B: Re-export Pattern (${#category_b[@]} files)" >> /tmp/duplicate-analysis.txt
for item in "${category_b[@]}"; do
    echo "  - $item" >> /tmp/duplicate-analysis.txt
done

echo "" >> /tmp/duplicate-analysis.txt
echo "CATEGORY C: Needs Manual Review (${#category_c[@]} files)" >> /tmp/duplicate-analysis.txt
for item in "${category_c[@]}"; do
    echo "  - $item" >> /tmp/duplicate-analysis.txt
done

cat /tmp/duplicate-analysis.txt
